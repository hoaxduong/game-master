    ...
    <div className="flex flex-col gap-6 sm:gap-8 p-4 sm:p-6 mx-auto min-h-screen pb-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {/* Game Over Banner */}
          {game.winner ? (
            <Card className="border-2 shadow-2xl rounded-3xl overflow-hidden bg-linear-to-br from-yellow-50 via-amber-50 to-orange-50 border-amber-300/60 dark:from-yellow-950 dark:via-amber-950 dark:to-orange-950 dark:border-amber-700/50">
              <CardContent className="p-8 sm:p-12">
                <div className="flex flex-col items-center text-center gap-6 animate-in fade-in zoom-in duration-500">
                  {/* Trophy */}
                  <div className="p-6 rounded-full bg-amber-200/60 text-amber-600 dark:bg-amber-800/40 dark:text-amber-300 shadow-xl">
                    <Trophy className="h-12 w-12" />
                  </div>

                  {/* Game Over label */}
                  <p className="text-sm font-bold uppercase tracking-[0.2em] text-amber-600/80 dark:text-amber-400/60">
                    {t["game.over"]}
                  </p>

                  {/* Winner text */}
                  <h2 className="text-3xl sm:text-4xl font-black text-amber-900 dark:text-amber-100 leading-tight">
                    {t[`game.winner.${game.winner}` as keyof typeof t]}
                  </h2>

                  {/* Winner icon */}
                  <div className="text-4xl">
                    {game.winner === "village" && "🏘️"}
                  ...
                  <div className="w-full max-w-md space-y-3 pt-4">
                    <p className="text-xs font-bold uppercase tracking-widest text-amber-700/60 dark:text-amber-300/50">
                      {t["game.survivors"]}
                    </p>
                    <div className="flex flex-wrap justify-center gap-2">
                      {players
                        ...
                        .map((p) => (
                          <Badge
                            key={p.id}
                            className="px-4 py-2 rounded-xl text-sm font-bold bg-emerald-100 text-emerald-700 border border-emerald-300/50 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-700/50"
                          >
                    ...
                    {players.filter((p) => !p.isAlive).length > 0 && (
                      <div className="flex flex-wrap justify-center gap-1.5 pt-2 opacity-50">
                        {players
                          .filter((p) => !p.isAlive)
                          .map((p) => (
                            <span
                              key={p.id}
                              className="text-xs font-medium px-2 py-0.5 rounded-lg border bg-muted/30 border-muted-foreground/10 line-through"
                            >
                              {p.name}
                            </span>
                          ))}
                  ...
                  <Button
                    ...
                    className="h-14 rounded-2xl font-black text-base px-8 shadow-xl bg-amber-500 text-amber-950 hover:bg-amber-400 shadow-amber-500/40 mt-4"
                  >
                              ...
                              {targetable.length > 0 && (
                                <div className="space-y-3">
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    {targetable.map((p) => {
                                      const isSelected = stepTargets.includes(
                                        p.id,
                                      );
                      ...
                      {currentStep.type === "mapping" && currentStep.roleId && (
                        <div className="w-full space-y-4 pt-4">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {players.map((p) => {
                              const isThisRole =
                                p.roleId === currentStep.roleId;
                              const hasOtherRole = p.roleId && !isThisRole;
                              const currentRoleCount = players.filter(
                                (player) =>
                                  player.roleId === currentStep.roleId,
                              ).length;
